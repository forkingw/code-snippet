<template>
  <page-header-wrapper>
    <a-card :bordered="false">
      <div class="alert-message" v-if="[-1, 0, 1].includes(alertStatus.status)">
        <a-alert
          v-bind="{...alertStatus}"
          show-icon
        >
    ...
    <DisasterFrontDetail ref="disasterFrontDetailRef" />
  </page-header-wrapper>
</template>

<script>
import { getDisasterFrontReport, getPropertyList, exportDisasterFrontList } from '@/api/contingencyManage/preDisasterPreparation.js'
import { glbArea } from '@/api/common.js'
import { STable, Ellipsis } from '@/components'
import { fileDownClick } from '@/utils/util.js'
import DisasterFrontDetail from './modules/DisasterFrontDetail.vue'
export default {
      ...
      columns: [
        ...
        },
        {
          ...
          customRender (text, record) {
            return record.floor ? `${text}/${record.floor}` : text
          }
        },
        {
          title: '物业公司',
  ...
  computed: {
    alertStatus () {
      ...
      switch (status) {
        ...
        case 1:
          type = 'success'
          message = `灾前上报进行中，距离上报结束还有${intervalStr}`
          break
        case -1:
          type = 'warning'
          message = '灾前上报已结束'
  ...
  methods: {
    ...
    getDisasterFrontInfo () {
      getDisasterFrontReport(this.noticeId).then(res => {
        if (res.code === 0) {
    ...
    loadData (parameter) {
      return getPropertyList(Object.assign({}, parameter, this.queryParam, {
    ...
    lookupNotice ({ floorId }) {
      this.$refs.disasterFrontDetailRef.showDrawer(floorId, this.noticeId)
    },
    exportDisasterList () {
      ...
      exportDisasterFrontList(params).then(res => {
        fileDownClick({
          res,
          fileName: '灾前准备.xls'
        })
