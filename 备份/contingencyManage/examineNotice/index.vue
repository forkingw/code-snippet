<template>
  <page-header-wrapper>
    <a-card :bordered="false">
      <div class="table-page-search-wrapper" @keyup.enter="handleQuery()">
        <a-form layout="inline">
          <a-row type="flex" :gutter="48">
            ...
            <a-col v-bind="formColSpan">
              <a-form-item label="发送状况">
                <a-select
                  placeholder="请选择"
                  allowClear
                  v-model="queryParam.publishStatus"
                >
                  <a-select-option v-for="(item) in publishStatusList" :value="item.value" :key="item.value">{{ item.label }}</a-select-option>
                </a-select>
              </a-form-item>
            </a-col>
            <a-col>
      ...
      <a-radio-group style="margin-top: 15px;margin-bottom: 15px;" button-style="solid" v-model="cmNoticeStatusId" @change="handleQuery">
      ...
      >
        ...
        <template slot="action" slot-scope="text, record">
          <a-space>
            ...
            <a v-if="(isPropertySectionAcc || isSysAccount) && record.cmNoticeStatusId === 1" @click="toExamine(record)" v-action:pass>审核</a>
          </a-space>
        </template>
      </s-table>
    </a-card>
    <ExamineDrawer ref="examineDrawerRef" @handleQuery="handleQuery" />
  </page-header-wrapper>
</template>

<script>
import { getVerifyList } from '@/api/contingencyManage/examineNotice.js'
...
import ExamineDrawer from './modules/ExamineDrawer.vue'
...
export default {
  ...
  methods: {
    ...
    loadData (parameter) {
      return getVerifyList(Object.assign({}, parameter, this.queryParam, {
        areaId: this.area.slice(-1)[0],
        cmNoticeStatusId: this.cmNoticeStatusId
      })).then(res => {
     ...
     */
     toExamine ({ noticeId }) {
      this.$refs.examineDrawerRef.showDrawer(noticeId)
     }
  }
}
</script>
